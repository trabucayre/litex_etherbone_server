...
int etherbone_callback(struct etherbone_memory_handlers *mem,
        uint8_t *buf, uint32_t buf_size, uint32_t recv_len)
{
    ...
    if (packet == NULL) {
        fprintf(stderr, "Could not find packet magic: recv_len=%u\n", recv_len);
        DUMP((struct etherbone_packet *) buf);
        return -1;
    }

    uint32_t bytes_available = recv_len - offset;
    if (bytes_available < ETHERBONE_HEADER_LENGTH) {
        fprintf(stderr, "Not enought bytes for EtherBone header: available=%u\n", bytes_available);
        DUMP(packet);
        return -1;
    }

    uint32_t packet_len = ETHERBONE_HEADER_LENGTH;
    if (packet->pf == 0) {
    ...
    if (bytes_available < packet_len) {
        fprintf(stderr, "Received less bytes than packet_len: len=%u, available=%u\n",
    ...
    if (req_packet == NULL) {
        fprintf(stderr, "Could not allocate buffer for packet: len=%u\n", packet_len);
        DUMP(packet);
        return -1;
    }
